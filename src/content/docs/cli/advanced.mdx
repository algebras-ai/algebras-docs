...
Glossaries can be created through the platform, or uploaded directly from a CSV, TSV, or XLSX file using `algebras glossary push`:

```bash
algebras glossary push terms.csv --name "Product Glossary"
```

This creates a new glossary and uploads the terms in batches (100 per batch by default, adjustable with `--batch-size`). The command prints the new glossary's ID, which you'll use with `--glossary-id`. Other options:

- `--rows-ids <ids>` - Upload only specific rows (comma-separated row numbers or record IDs)
- `--max-length <number>` - Skip terms longer than this (default: 255)
- `--debug` - Log outgoing requests before sending
...
Or set it as a project default so every `translate` run uses it automatically:

```bash
algebras configure --glossary-id glossary-123
```
...
## Forcing and Caching Behavior

`translate` normally skips files/keys that are already up to date and uses translation caching. A few flags override that behavior for specific cases:

### Force Re-translating Specific Keys

```bash
# Force everything to re-translate
algebras translate --force

# Force only specific keys, leaving the rest untouched
...
```bash
# Skip both the API-side and local translation cache
algebras translate --no-cache
```

Useful when a previous translation was wrong and you need a genuinely fresh result rather than a cached one.

### Regenerating Files From Scratch

By default, `translate` updates existing target files in place. To discard the existing target file and regenerate it entirely:
...
`translate` submits jobs to an async submit-and-poll endpoint by default. If you need the older blocking behavior:
...
# CI-friendly check — exits non-zero if translations are missing or outdated
algebras ci
```
...
```bash
# Translate a single language at a time
...
Shows, per configured language:
- Translated vs. expected key counts and completion percentage
- Outdated key counts
- Translated vs. expected file counts
- Warnings for language codes the API doesn't recognize
  ...
  translate:
    ...
    steps:
      ...
      - name: Check Translations
        run: algebras ci --only-missing
        env:
          ALGEBRAS_API_KEY: ${{ secrets.ALGEBRAS_API_KEY }}
```

`algebras ci` already exits with a non-zero status on missing/outdated translations, which fails the job — no extra flag needed.
