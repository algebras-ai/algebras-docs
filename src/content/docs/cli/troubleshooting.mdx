...
The CLI does not write log files — all output goes to your terminal (stdout/stderr). Re-run the failing command with `--verbose` and capture the output if you need to share it (e.g. `algebras translate --verbose > algebras.log 2>&1`).
