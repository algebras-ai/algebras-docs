...
```yaml
api:
  provider: algebras-ai  # Default and currently only supported provider
```
...
`batch_size` and `max_parallel_batches` are top-level config keys (not nested under `api`), matching what `algebras configure --batch-size`/`--max-parallel-batches` write.
...
`add` takes one language per invocation. There is no `algebras remove` command — remove a language by deleting its entry from `languages` in `.algebras.config`.
